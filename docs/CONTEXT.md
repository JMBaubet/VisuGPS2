...
### Pages
- **Accueil** (`/`) : Fenêtre principale — drawer gauche (liste circuits triée par distance au centre de la carte), carte Mapbox (clusters de points de départ, popups ; **centrage/zoom mémorisés** dans les paramètres cachés `Carte.Vue.*` et restitués au lancement / au retour sur la vue), drawer droit (paramètres)

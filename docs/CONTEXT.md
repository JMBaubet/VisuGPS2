...
- **Nettoyage** (`/nettoyage`) : Nettoyage de trace GPX — carte Mapbox (trace complète en vert, segment courant surligné, linestring corrigé jaune, branches aller/retour décalées et colorées, points numérotés cliquables avec anti-revouvrement, sélecteur des points proches du curseur pour les points superposés, points supprimés en rouge, **déplacement direct à la souris** des points des cas manuels), panneau des anomalies (validation manuelle « Corriger & valider » / « Conserver tel quel » pour chaque cas, bouton **Créer une anomalie** pour désigner manuellement un segment, **Supprimer ce cas** pour les cas manuels non validés), table simplifiée des points du segment (numéro, suppression, indicateur « Déplacé »), boutons **Enregistrer** (sauvegarde partielle) et **Finaliser** (actif quand tous les cas sont validés — génère le GPX nettoyé + backup `.orig`). Accessible depuis le bouton Éditer d'un circuit « À nettoyer » (badge orange, icône `mdi-broom`) et via le garde-fou d'`EditionCamera`. Une trace non « clean » ne peut pas entrer en édition caméra.
...
- `src/stores/cleaning.ts` : état de la vue de nettoyage de trace (pattern Setup Store). Expose `selectedTraceId` (posé avant la navigation vers `/nettoyage`), `state` (détection + décisions, persisté dans `cleaning/{trace_id}.json`), `points` (index GPX), `currentCaseIndex`, `toleranceDeg` (paramètre `Nettoyage.Cap.toleranceDeg`), l'état UI éphémère `createMode`/`createStartIndex`/`movePointIndex` (création manuelle et déplacement), les getters `hasCases`, `currentCase`, `currentZone`, `correctedZoneCoords` (coordonnées `[lon, lat]` du segment courant **après corrections** — points supprimés retirés, points déplacés repositionnés ; alimente le linestring jaune de la carte), `allValidated` (tous les cas ≠ `pending`), `validatedCount`, `isDeletedCount`, et les actions `load` (reprise du travail en cours via `get_cleaning_state`, sinon détection via `detect_trace_anomalies`), `reDetect`, `goToCase`, `applySuggestion` (pré-remplit la suppression suggérée), `toggleDeletePoint`/`addDeleteRange`/`clearCorrection` (suppressions), `setMovedPoint`/`clearMovedPoint`/`startMovePoint`/`stopMovePoint` (déplacement géographique des points), `toggleCreateMode`/`cancelCreate`/`createManualCase`/`removeCase` (cas **manuels** créés à la carte, supprimables avant validation), `validateCurrentCase('corrected' | 'kept')` (validation manuelle « Corriger & valider » / « Conserver tel quel »), `save` (sauvegarde partielle, passe la trace en `in_progress`), `finalize` (n'exécute que si tous les cas sont validés), `reset` (abandon → `needs_review`).

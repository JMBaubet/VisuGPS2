...
Le **workflow principal** de l'application est : **import GPX → audit (si anomalies) → édition caméra**. Une trace n'est **valide** que si elle est auditée ; les traces importées présentant des anomalies (aller-retours ponctuels, boucles de giratoire) doivent être auditées via la vue `/audit` avant de pouvoir entrer en édition caméra.
...
│   ├── router/
│   │   └── index.ts          # Routes (Accueil, Visualisation, EditionCamera, Audit, ScreenBis)
│   ├── stores/
...
│   │   ├── audit.ts          # Store du module Audit GPX (détection AR/RP, findings volatils, corrections)
...
│   │   ├── gpx_audit/        # Module Audit GPX (détecteurs AR/RP, corrections, export, commandes)
...
**Audit GPX** (détection AR + RP, corrections, réécriture GPX ; les findings sont **volatils** — aucune persistance) :
...
- `audit_validate(traceId, points, findings)` : **Point de non-retour** — refuse tant qu'un finding est `pending`, réécrit le GPX (backup `{filename}.gpx.orig` une seule fois), régénère geojson/stats/hash et pose `audit_status = "clean"`
...
- `src/stores/audit.ts` : état du module Audit GPX (pattern Setup Store, état **volatil** non persisté). Expose les types miroir des structs Rust (`AuditParams`, `AuditPoint`, `Finding`, `FindingKind`, `FindingStatus`, `AuditState`, `AuditDetectionResult`, `DeletePreview`, `FindingOverlay`), l'état `traceId`, `points` (trace de travail), `findings`, `params`, `selectedFindingId`, les aperçus (`deletePreview`, `routePreview`), les getters de progression (`hasWorkInProgress`, compteurs d'anomalies traitées / restantes), et les actions `runAudit(traceId, params)` (détection initiale via `audit_run_detection`), `selectFinding`, `applyDelete`, `applyRoute`, `markFp`, `unmarkFp`, `undoCorrection`, `clearDeletePreview`, `clearRoutePreview`, `setRouteRange`, **`validate`** (bouton « Appliquer » : refuse tant qu'un finding est `pending`), `reset` (réinitialisation à la sortie de la vue — décision 9). Les appels réseau OpenRouteService vivent dans la composable `src/composables/useAuditOrs.ts`.
...
mod settings;
mod import_gpx;
mod gpx_audit;   // module Audit GPX (14 fichiers : types, détecteurs, corrections, commandes)

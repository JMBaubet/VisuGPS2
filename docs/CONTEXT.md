...
- **Nettoyage** (`/nettoyage`) : Nettoyage de trace GPX — carte Mapbox (trace en filigrane, segment courant surligné, branches aller/retour décalées et colorées, points numérotés cliquables, ajout de points par clic), panneau des anomalies (validation manuelle « Corriger & valider » / « Conserver tel quel » pour chaque cas), table des points du segment, boutons **Enregistrer** (sauvegarde partielle) et **Finaliser** (actif quand tous les cas sont validés — génère le GPX nettoyé + backup `.orig`). Accessible depuis le bouton Éditer d'un circuit « À nettoyer » (badge orange, icône `mdi-broom`) et via le garde-fou d'`EditionCamera`. Une trace non « clean » ne peut pas entrer en édition caméra.
...
- `src/stores/cleaning.ts` : état de la vue de nettoyage de trace (pattern Setup Store). Expose `selectedTraceId` (posé avant la navigation vers `/nettoyage`), `state` (détection + décisions, persisté dans `cleaning/{trace_id}.json`), `points` (index GPX), `currentCaseIndex`, `toleranceDeg` (paramètre `Nettoyage.Cap.toleranceDeg`), les getters `hasCases`, `currentCase`, `currentZone`, `correctedZoneCoords` (coordonnées `[lon, lat]` du segment courant **après corrections** — points supprimés retirés, points ajoutés intégrés ; alimente le linestring jaune de la carte), `allValidated` (tous les cas ≠ `pending`), `validatedCount`, `isDeletedCount`, et les actions `load` (reprise du travail en cours via `get_cleaning_state`, sinon détection via `detect_trace_anomalies`), `reDetect`, `goToCase`, `applySuggestion` (pré-remplit la suppression suggérée), `toggleDeletePoint`/`addDeleteRange`/`clearCorrection` (suppressions), `addInsertPoint`/`removeInsertPoint`/`setInsertPointPosition` (ajouts — cas « parallel »), `validateCurrentCase('corrected' | 'kept')` (validation manuelle « Corriger & valider » / « Conserver tel quel »), `setCaseKind`, `save` (sauvegarde partielle, passe la trace en `in_progress`), `finalize` (n'exécute que si tous les cas sont validés), `reset` (abandon → `needs_review`).

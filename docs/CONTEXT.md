...
- `get_cleaning_state(traceId, phase, toleranceDeg, roundaboutParams)` : Retourne le travail de la phase (`cleaning/{trace_id}.{phase}.json`), sinon une détection fraîche
...
- **Nettoyage** (`/nettoyage`) : Nettoyage de trace GPX en **3 étapes** avec **widget « boîte à états »** dans la toolbar (1 Pts hors trace → 2 Rond-Points → 3 Aller/Retour, ✓/✗, navigation séquentielle) — carte Mapbox (trace complète en vert, segment courant surligné, linestring corrigé jaune, branches aller/retour décalées et colorées, points numérotés cliquables avec anti-revouvrement, sélecteur des points proches du curseur, points supprimés en rouge, déplacement direct à la souris des points des cas manuels), panneau des anomalies (validation manuelle « Valider » / « Faux positif », **poubelle rouge** sauf pour les cas « Aller-retour », bouton **« Modifier un segment »**, affichage **angle cumulé / tours / sens** pour les ronds-points), table simplifiée des points du segment (numéro, suppression avec case d'en-tête tout supprimer / tout remettre, indicateur « Déplacé », **zone élargie d'une marge pour les ronds-points**), boutons **Enregistrer** (sauvegarde partielle) et **Valider l'étape** (applique la phase, réécrit le GPX, passe à l'étape suivante). L'étape 3 « Aller/Retour » est **non implémentée** (simple emplacement) : la trace reste `needs_review`. Accessible depuis le bouton Éditer d'un circuit « À nettoyer » (icône `mdi-broom`) et via le garde-fou d'`EditionCamera`. Une trace non « clean » ne peut pas entrer en édition caméra.
...
- **Nettoyage de trace GPX** en **3 étapes séquentielles** (Pts hors trace → Rond-Points → Aller/Retour), avec **widget « boîte à états »** dans la toolbar (✓/✗ par étape, navigation séquentielle) : étape 1 = points isolés hors trace (rebroussement ~180°, tolérance `Nettoyage.Cap.toleranceDeg`) ; étape 2 = **ronds-points** (tours soutenus — cumul d'angle, paramètres `Nettoyage.RondPoints.*`, correction manuelle dans une zone élargie d'une marge) ; étape 3 = aller/retour (**non implémentée**, simple emplacement — produira un autre type de fichier). Validation d'étape = réécriture du GPX (entrée de l'étape suivante), sauvegardes partielles (`cleaning/{trace_id}.{phase}.json`), backup `{filename}.gpx.orig` une seule fois. La trace **reste `needs_review`** tant que l'étape 3 n'existe pas. Une trace non « clean » ne peut pas entrer en édition caméra.

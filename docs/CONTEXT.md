...
│   │   ├── cleaning.rs       # Nettoyage de trace GPX (détection d'anomalies, persistance, finalisation)
...
**Nettoyage de trace** :
- `detect_trace_anomalies(traceId, toleranceDeg)` : Détecte les anomalies du GPX original (points hors trace, aller-retours — changement de cap ~180°, tolérance `Nettoyage.Cap.toleranceDeg`), sans persistance
- `get_cleaning_state(traceId, toleranceDeg)` : Retourne le travail de nettoyage en cours (`cleaning/{trace_id}.json`), sinon une détection fraîche
- `save_cleaning_state(traceId, stateJson)` : Sauvegarde partielle des corrections (écriture atomique) et passe la trace en `in_progress`
- `reset_cleaning(traceId)` : Abandonne les corrections en cours (supprime le fichier) et repasse en `needs_review`
- `finalize_cleaning(traceId, stateJson)` : Applique les corrections validées, génère le GPX nettoyé + backup `{filename}.gpx.orig`, régénère geojson/stats/hash, repasse en `clean` (refuse tant que des cas restent `pending`)
...
- **Nettoyage** (`/nettoyage`) : Nettoyage de trace GPX — carte Mapbox (trace complète en vert, segment courant surligné, linestring corrigé jaune, branches aller/retour décalées et colorées, points numérotés cliquables avec anti-revouvrement, sélecteur des points proches du curseur pour les points superposés, points supprimés en rouge, **déplacement direct à la souris** des points des cas manuels), panneau des anomalies (validation manuelle « Valider » / « Faux positif » pour chaque cas, **poubelle rouge** — sauf pour les cas « Aller-retour » —, bouton **« Modifier un segment »** sous la liste pour désigner manuellement un segment, bouton **« Restaurer »** sur la même ligne que Valider / Faux positif), table simplifiée des points du segment (numéro, suppression avec **case d'en-tête tout supprimer / tout remettre**, indicateur « Déplacé »), boutons **Enregistrer** (sauvegarde partielle) et **Finaliser** (actif quand tous les cas sont validés — génère le GPX nettoyé + backup `.orig`). Accessible depuis le bouton Éditer d'un circuit « À nettoyer » (icône `mdi-broom`) et via le garde-fou d'`EditionCamera`. Une trace non « clean » ne peut pas entrer en édition caméra.
...
- **Nettoyage de trace GPX** : détection automatique des anomalies à l'import (points hors trace, aller-retours — changement de cap ~180°, tolérance paramétrable `Nettoyage.Cap.toleranceDeg`), correction manuelle segment par segment dans la vue `/nettoyage`, sauvegardes partielles (`cleaning/{trace_id}.json`), finalisation avec GPX nettoyé + backup `{filename}.gpx.orig`. Une trace non « clean » ne peut pas entrer en édition caméra.
...
- `src/stores/cleaning.ts` : état de la vue de nettoyage de trace (pattern Setup Store). Expose `selectedTraceId` (posé avant la navigation vers `/nettoyage`), `state` (détection + décisions, persisté dans `cleaning/{trace_id}.json`), `points` (index GPX), `currentCaseIndex`, `toleranceDeg` (paramètre `Nettoyage.Cap.toleranceDeg`), l'état UI éphémère `createMode`/`createStartIndex`/`movePointIndex` (modification de segment et déplacement), les getters `hasCases`, `currentCase`, `currentZone`, `correctedZoneCoords` (coordonnées `[lon, lat]` du segment courant **après corrections** — points supprimés retirés, points déplacés repositionnés ; alimente le linestring jaune de la carte), `allValidated` (tous les cas ≠ `pending`), `validatedCount`, `isDeletedCount`, `isZoneFullyDeleted`, et les actions `load` (reprise du travail en cours via `get_cleaning_state`, sinon détection via `detect_trace_anomalies`), `reDetect`, `goToCase`, `toggleDeletePoint`/`addDeleteRange`/`clearCorrection`/`setZoneDeleted` (suppressions — la suggestion de détection est appliquée automatiquement à la validation), `setMovedPoint`/`clearMovedPoint`/`startMovePoint`/`stopMovePoint` (déplacement géographique des points), `toggleCreateMode`/`cancelCreate`/`createManualCase`/`removeCase` (cas « Modification de segment », **supprimables même après validation**), `validateCurrentCase('corrected' | 'kept')` (validation manuelle « Valider » / « Faux positif »), `save` (sauvegarde partielle, passe la trace en `in_progress`), `finalize` (n'exécute que si tous les cas sont validés), `reset` (abandon → `needs_review`).

...
1. **Génération des keyframes** (`src/algorithms/keyframeGenerator.ts`) :
   ...
   - `generateKeyframes(traceId, feature, sampleStepM = 250, tracePoints?, algorithm = 'frustum', minKeyframeGapM = 1000, terrainSampler?)` — deux algorithmes au même format de sortie :
...
5. **Composants** (`src/components/Edition/`) :
   - `EditionToolbar.vue` : `v-app-bar` semi-transparente. Bouton **Home**, titre de la trace, toggle **cadre ViewPort**, **sélecteur d'algorithme** (Frustum / Simple) et champ **Gap min (m)** (bornes 200–5000, pas 50) — reliés au store, toute modification régénère et re-persiste les keyframes.

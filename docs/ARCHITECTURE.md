...
2. **Store Frontend** (`src/stores/cleaning.ts`) — Pattern Setup Store :
   ...
   - Actions : `load` (reprise du travail en cours via `get_cleaning_state`, sinon détection via `detect_trace_anomalies`), `reDetect`, validation manuelle des cas (« Valider » / « Faux positif » — la suppression suggérée est appliquée automatiquement à la validation), `toggleDeletePoint`/`addDeleteRange`/`clearCorrection`/`setZoneDeleted`/`isZoneFullyDeleted` (suppressions), `setMovedPoint`/`clearMovedPoint`/`startMovePoint`/`stopMovePoint` (déplacements), `toggleCreateMode`/`cancelCreate`/`createManualCase`/`removeCase` (cas manuels « Modification de segment », **supprimables même après validation**), `save` (sauvegarde partielle via `save_cleaning_state`, le GPX original reste intact), `finalize` (`finalize_cleaning`), `reset` (`reset_cleaning`).

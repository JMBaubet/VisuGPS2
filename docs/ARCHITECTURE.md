...
3. **Store Frontend** (`src/stores/traces.ts`) :
   ...
   - État `traceToFrameId` : id de la trace à **cadrer** après un import — signal **one-shot** posé par `importerGpx` et consommé par `Map.vue` (cf. §5, « Cadrage à l'import »).
...
5. **Carte Mapbox** (`src/components/Accueil/Map.vue`) :
   ...
   - **Cadrage à l'import** : `watch(tracesStore.traceToFrameId)` → `getTraceGeometry` puis `fitBounds` avec une marge de 10 % de chaque côté (la trace occupe au plus 80 % de la carte). Le signal est **one-shot**, et son contrat est tenu par le consommateur : tant que la carte n'est **pas prête** (`mapReady` faux) il **reste armé**, et le cadrage est repris par le handler `load` — le watch n'étant pas immédiat, un signal posé avant que la carte soit prête serait sinon perdu et resterait armé ; dès qu'un cadrage est **tenté**, en revanche, le signal est consommé, échec compris, ses causes étant durables (géométrie illisible, GPX disparu) et un second essai devant échouer à l'identique.

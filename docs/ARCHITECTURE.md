...
5. **Composants** (`src/components/Edition/`) :
   - `EditionToolbar.vue` : `v-app-bar` semi-transparente. Bouton **Home**, titre de la trace, **menu ViewPort** (sélection du ratio 16:9 / 4:3 — chaque ratio exploite son propre fichier keyframes — + affichage/masquage du cadre), **sélecteur d'algorithme** (Frustum / Simple) et champ **Gap min (m)** (bornes 200–5000, pas 50) — reliés au store, toute modification régénère et re-persiste les keyframes.

...
5. **Composants** (`src/components/Edition/`) :
   - `EditionToolbar.vue` : `v-app-bar` semi-transparente. Bouton **Home**, titre de la trace, **bouton ViewPort** (**flip-flop** 16:9 / 4:3 — icône `mdi-monitor` en 16:9 / `mdi-monitor-small` en 4:3, chaque ratio exploite son propre fichier keyframes et son cadre ; **couleur** = avancement du verrouillage : vert 100 %, jaune > 50 %, orange ≥ 10 %, rouge sinon), **sélecteur d'algorithme** (Frustum / Simple) et champ **Gap min (m)** (bornes 200–5000, pas 50) — reliés au store, toute modification régénère/re-persiste les keyframes — plus un bouton **Cap brutaux** qui affiche/masque le panneau tableau, et un bouton **Mode validation** (`mdi-camera-lock`, surligné quand actif) qui arme le verrouillage automatique des segments pendant la lecture.

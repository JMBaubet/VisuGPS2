...
│   │   ├── CleaningCasesPanel.vue # Panneau (liste des cas + poubelle, bouton « Modifier un segment », validation « Valider » / « Conserver tel quel »)
...
2. **Store Frontend** (`src/stores/cleaning.ts`) — Pattern Setup Store :
   ...
   - Actions : `load` (reprise du travail en cours via `get_cleaning_state`, sinon détection via `detect_trace_anomalies`), `reDetect`, validation manuelle des cas (« Valider » / « Conserver tel quel » pour les faux positifs — la suppression suggérée est appliquée automatiquement), `toggleDeletePoint`/`addDeleteRange`/`clearCorrection`/`setZoneDeleted`/`isZoneFullyDeleted` (suppressions), `setMovedPoint`/`clearMovedPoint`/`startMovePoint`/`stopMovePoint` (déplacements), `toggleCreateMode`/`cancelCreate`/`createManualCase`/`removeCase` (cas manuels « Modification de segment », **supprimables même après validation**), `save` (sauvegarde partielle via `save_cleaning_state`, le GPX original reste intact), `finalize` (`finalize_cleaning`), `reset` (`reset_cleaning`).
...
4. **Composants** (`src/components/Cleaning/`) :
   ...
   - `CleaningCasesPanel.vue` : liste des anomalies (n°/total validés, type, écart de cap, zone) avec **poubelle rouge** par cas (supprimable : cas « Modification de segment » toujours, cas détectés tant que non validés), bouton **« Modifier un segment »** placé **sous la liste** (désignation d'un segment sur la carte) et validation de chaque cas (« Valider » / « Conserver tel quel »).
